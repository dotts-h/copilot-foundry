...
- **CPython `.pyc` bytecode-cache staleness can make pytest silently execute stale code.** Verified live during Task 8 (2026-07-01, root-caused with a bare-Python repro outside any project code): the default timestamp-based `.pyc` invalidation only stores whole-second mtimes, so two writes to the same `.py` file within the same wall-clock second produce a cache CPython considers unchanged — the second `pytest` run silently reuses bytecode compiled from the FIRST write. This is a real correctness gap in shared M0 infrastructure (`pythonRunner.ts`/`exec.ts`), not something scoped to one task: any tight rewrite-then-rerun loop (GREEN's repair loop being the first to actually hit it) is exposed. **Fixed at the source, not patched per-caller:** `exec.ts`'s `RunCommandOptions` gained an optional `env?: Record<string,string>` field (passed to `spawn` merged over `process.env`; omitted `env` is unchanged behavior — backward compatible with every existing M0 caller), and `pythonRunner.ts`'s `runPytest` now always passes `env: { PYTHONDONTWRITEBYTECODE: "1" }` so no `.pyc` is ever written in a target repo, so there is never a stale cache to hit. This is a deliberate, justified exception to "M0 primitives reused unchanged" — a genuine bug found via real-signal root-causing overrides that constraint, per this repo's own coding principles (verify on real signal, don't guess twice).
