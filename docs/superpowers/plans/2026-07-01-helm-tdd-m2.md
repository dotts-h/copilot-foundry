...
**CORRECTION (found live during Task 5's implementation, not anticipated when this plan was written): the two multi-slice regression tests need their `ScriptedBackend` scripts extended, not just their status assertions left alone.** `"reports completed_with_regressions when a later slice's GREEN implementation breaks an already-passing baseline test..."` and `"reports completed_with_regressions when slice 2's GREEN implementation breaks slice 1's own newly-authored, already-committed test"` each hand-script exactly 5 backend calls (plan, RED1, GREEN1, RED2, GREEN2) assuming NO backend call happens between GREEN1 and RED2. REFACTOR now inserts exactly one backend call after every slice's GREEN, so the real call sequence is plan, RED1, GREEN1, REFACTOR1, RED2, GREEN2, REFACTOR2 — the REFACTOR1 call silently consumes the script step that was meant for RED2 (writing test_subtract_kata.py), which desyncs everything after it and makes slice 2's RED gate see subtract already implemented (`already_green`) instead of exercising the intended regression scenario. Fix: insert one explicit no-op REFACTOR step (`async () => {}` — a valid simulation of "the model decided nothing needed refactoring", harmless: `attemptRefactor` measures identical before/after metrics on an untouched file, the ratchet holds trivially, `refactorApplied: true`) immediately after EACH slice's GREEN step in BOTH tests' `ScriptedBackend` arrays (i.e. after index 2 and after the final GREEN step — 7 steps total per test, not 5). The tests' assertions themselves (`ledger.sliceResults` length, `greenGatePassed`, final `status`) do not need to change — only the script arrays need the two no-op steps inserted at the right positions.
